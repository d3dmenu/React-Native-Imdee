...
//////////////////////// ok ////////////////////////////////////////////////
  ...
  async uploadImage(ref, uri, imageName)
  {
    if(uri=='' || uri==null || uri==undefined){
      uri = "https://www.img.in.th/images/987fa234253fb2281a66a31ee839a528.jpg";
    }
    var metadata = {
      contentType: "image/jpeg",
    };
          ...
          if(date1 == date2 && Number(doc.data().Amount) == Number(obj.Amount) && doc.data().Type == obj.Type){
            if(doc.data().Type == 'SCB'){//check name
              ...
              if(checkN1==true && checkN2==true){
                await firebase.firestore().collection('SE_Restaurant/IMDEE/Wallet').doc(user).get().then(async docs=>{
                  if(docs.exists){
                    await firebase.firestore().collection('SE_Restaurant/IMDEE/Wallet').doc(user).update({Money:docs.data().Money+Number(doc.data().Amount), Income:docs.data().Income+Number(doc.data().Amount)});
                  }
                });
                var data = {
                ...
                await firebase.firestore().collection('SE_Restaurant/IMDEE/Wallet/'+user+'/History').add(data);
                await firebase.firestore().collection('BankClient').doc(doc.id).delete();
                check = true;
